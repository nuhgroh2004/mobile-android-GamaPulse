...
class ProfilActivity : AppCompatActivity() {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                withContext(Dispatchers.Main) {
                    ...
                    if (response.isSuccessful) {
                        ...
                        val errorDialog = SweetAlertDialog(this@ProfilActivity, SweetAlertDialog.ERROR_TYPE)
                            .setTitleText("Gagal!")
                            .setContentText("Gagal memperbarui profil: ${response.code()} ${response.message()}")
                            .setConfirmClickListener { it ->
                                it.dismissWithAnimation()
                            }
                        errorDialog.show()
                ...
                withContext(Dispatchers.Main) {
                    ...
                    val errorDialog = SweetAlertDialog(this@ProfilActivity, SweetAlertDialog.ERROR_TYPE)
                        .setTitleText("Error!")
                        .setContentText("Gagal memperbarui profil: ${e.message}")
                        .setConfirmClickListener { it ->
                            it.dismissWithAnimation()
                        }
                    errorDialog.show()
            ...
            try {
                ...
                withContext(Dispatchers.Main) {
                    ...
                    if (response.isSuccessful) {
                        ...
                        Toast.makeText(
                            this@ProfilActivity,
                            "Server error: ${response.code()}, logged out locally",
                            Toast.LENGTH_SHORT
                        ).show()
                        performLocalLogout()
                    }
    ...
    private fun saveUserData(dialog: SweetAlertDialog) {
        ...
        val password = binding.etPassword.text.toString()
        val passwordToSend = if (password.isNotEmpty() && password != "********") password else null
